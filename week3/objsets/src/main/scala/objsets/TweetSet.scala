...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = this.mostRetweetedAcc(this.elem)

  override def mostRetweetedAcc(mostSoFar: Tweet): Tweet =
    if (mostSoFar.retweets < elem.retweets)
      right.mostRetweetedAcc(left.mostRetweetedAcc(elem))
    else
      left.mostRetweetedAcc(right.mostRetweetedAcc(mostSoFar))

  override def leastRetweeted: Tweet = this.leastRetweetedAcc(this.elem)

  override def leastRetweetedAcc(leastSoFar: Tweet): Tweet =
    if (leastSoFar.retweets > elem.retweets)
      right.leastRetweetedAcc(left.leastRetweetedAcc(elem))
    else
      left.leastRetweetedAcc(right.leastRetweetedAcc(leastSoFar))
...
object GoogleVsApple {
  ...
  def checkFor(l: List[String]): Tweet => Boolean = x => l.contains(x)

  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter(checkFor(google))
  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter(checkFor(apple))
